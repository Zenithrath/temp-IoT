...
const SplineComponent = dynamic(() => import("@/components/SplineComponent"), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full flex items-center justify-center bg-cream-100/30 rounded-2xl border border-gray-200/50">
...
export default function Home() {
  ...
  const avgTemp = selectedDevice.history.reduce((sum, h) => sum + h.temp, 0) / selectedDevice.history.length;
  const avgHum = selectedDevice.history.reduce((sum, h) => sum + h.hum, 0) / selectedDevice.history.length;
  const minTemp = Math.min(...selectedDevice.history.map(h => h.temp));
  const maxTemp = Math.max(...selectedDevice.history.map(h => h.temp));
    ...
    <DashboardLayout>
      ...
      <div className="flex items-center justify-between gap-3 mb-3 sm:mb-5">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-xl shadow-card">
            <Cpu className="h-3 w-3 text-primary" />
            <span className="text-[10px] sm:text-xs font-bold text-gray-700">{totalDevices}</span>
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse ml-0.5" />
          </div>
          <div className="hidden sm:flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-xl shadow-card">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-12 gap-3 sm:gap-4 mb-4 sm:mb-5">
        ...
        <div className="col-span-1 lg:col-span-7 flex">
          <div className="h-[145px] sm:h-[220px] lg:h-[300px] w-full relative rounded-2xl overflow-hidden bg-white shadow-card border border-gray-200/60">
            <SplineComponent />
            <div className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3 bg-white/80 backdrop-blur-md px-2 py-1 rounded-lg shadow-card text-[9px] sm:text-[10px] font-bold text-gray-500 flex items-center gap-1.5 pointer-events-none">
        ...
        <div className="col-span-1 lg:col-span-5 flex flex-col">
          <div className="bg-white rounded-2xl p-3 sm:p-5 shadow-card border border-gray-200/60 h-[145px] sm:h-[220px] lg:h-[300px] flex flex-col justify-center sm:justify-between gap-2 sm:gap-0">
            <div className="flex items-center justify-between gap-1.5 sm:mb-3">
              ...
              <div className="relative">
                <button 
                  ...
                  className="flex items-center gap-1.5 bg-white border border-gray-200/60 hover:bg-gray-50 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-xl text-[9px] sm:text-[10px] font-bold text-slate-600 transition-colors"
                >
                  <Cpu className="h-3 w-3 text-primary" />
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-1 w-40 sm:w-48 bg-white rounded-xl shadow-card-elevated border border-gray-200/60 py-1 z-50">
                    {devices.map((d) => (
                      <button
                        key={d.id}
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <div className="bg-gray-50 rounded-xl p-2 sm:p-3 flex flex-col items-center sm:items-start border border-gray-200/60">
            ...
            <div className="hidden sm:flex items-center justify-between pt-2 border-t border-gray-200/60 mt-auto">
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <span className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-wider">Now</span>
              </div>
